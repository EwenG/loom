(ns loom.test.alg
  (:require [loom.graph :refer [weighted-graph weighted-digraph
                                weight digraph graph nodes
                                successors edges add-nodes
                                remove-nodes]]
            [loom.alg :refer [astar-path pre-traverse post-traverse
                              pre-span topsort bf-traverse bf-path
                              bf-path-bi dijkstra-path
                              dijkstra-path-dist dijkstra-traverse
                              dijkstra-span connected-components
                              strongly-connected? connected? connect
                              shortest-path loners dag? bellman-ford
                              bipartite-color bipartite? prim-mst-edges
                              prim-mst astar-dist bipartite-sets bf-span
                              scc]]
            #+clj [clojure.test :refer [deftest are is] :as t]
            #+cljs [cemerick.cljs.test :as t])
  #+cljs (:require-macros [cemerick.cljs.test :refer [deftest are is]]))
...
(deftest bellman-ford-test
  (are [expected graph start]
    ...
    false g11 :a
    false g11 :b
    [{:e #+clj Double/POSITIVE_INFINITY #+cljs js/Number.POSITIVE_INFINITY,
      :d #+clj Double/POSITIVE_INFINITY #+cljs js/Number.POSITIVE_INFINITY,
      :b #+clj Double/POSITIVE_INFINITY #+cljs js/Number.POSITIVE_INFINITY,
      :a #+clj Double/POSITIVE_INFINITY #+cljs js/Number.POSITIVE_INFINITY,
      :c 0}{:c [:c]}] g11 :c
    false g11 :d
    false g11 :e
    [{:e 10,
...
(deftest astar-path-test
  ...
  (is (thrown? #+clj Exception #+cljs js/Error (astar-path astar-with-unreachable-target-g2 :a :e nil))
      ))
...
(comment
  (t/test-ns 'loom.test.alg)
  (t/test-function loom.test.alg/prim-mst-test)
  )