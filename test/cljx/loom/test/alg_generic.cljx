(ns loom.test.alg-generic
  ...
  #+cljs (:require-macros [cemerick.cljs.test :refer [deftest are]]
                         ...
                         (fn [parents]
                           (if (< 0 nodes)
                             (gen-dag (conj dag-so-far (set parents))
                                      (dec nodes))
                             (dag-samples-gen dag-so-far #+clj 1/2 #+cljs 0.5))))))))
...
(comment
  (t/test-ns 'loom.test.alg-generic)
  (t/test-function loom.test.alg-generic/dag-similarity-100)
  )