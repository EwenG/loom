...
(defn- init-estimates
  ...
  (let [nodes (disj (nodes graph) start)
        path-costs {start 0}
        paths {start nil}
        infinities (repeat #+clj Double/POSITIVE_INFINITY #+cljs js/Number.POSITIVE_INFINITY)
        nils (repeat nil)
    ...
    (if (some
      ...
      [costs
       (->> (keys paths)
            ...
            (remove #(= #+clj Double/POSITIVE_INFINITY #+cljs js/Number.POSITIVE_INFINITY (get costs %)))
            (reduce
              (fn [final-paths v]
                (assoc final-paths v
...
(defn prim-mst-edges
  ...
  ([wg]
   (cond
     (directed? wg)
     #+clj (throw (Exception. "Spanning tree only defined for undirected graphs"))
     #+cljs (throw (js/Error. "Spanning tree only defined for undirected graphs"))
...
(comment
  ...
  (astar-path g2 :r :p nil)
  (astar-path g2 :r :p (fn [x y] 0) (pm/priority-map-keyfn first :r [0 nil 0 0]) {})
  )
...
(defn astar-path
  ...
  ([g src target heur q explored]
   (cond
     ...
     (empty? q)
     #+clj (throw (Exception. "Target not reachable from source"))
     #+cljs (throw (js/Error. "Target not reachable from source"))
     ;; target found, build path and return
     (= (first (peek q)) target) (let [u (first (peek q))      ;r
                                       parent ((second (peek q)) 1) ;nil
                                       explored(assoc explored target parent)    ;{:p nil}
               ...
               [curr-node (first (peek q))
                ...
                update-dist (fn [curr-node curr-dist q v]
                              (let [act (+ curr-dist
                                ...
                                (when (not (nil? (get q v)))
                                  #_(prn "ee "  [q v]
                                  #_(assoc q v [8 :b 8 0])
                                  #_(assoc q v [(+ act est ) curr-node act est])))
                                (cond
                                  (or (nil? (get q v))
                                      (> ((get q v) 2) act))
